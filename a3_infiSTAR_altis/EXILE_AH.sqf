...
FNC_A3_LOG_RAND_VAR = compileFinal "
	format['A3_RANDOMVAR_LOG:%1 (v0016F)',_this] call FN_CALL_LOG_DLL;
	diag_log ('<infiSTAR.de>RandomVarLog| '+_this+' (v0016F)');
	if(!isNil'MAR_fnc_log')then{[_this,'infiSTAR.de_RAND_VAR'] call MAR_fnc_log;};
";
FNC_A3_HACKLOG = compileFinal "
	format['A3_HACKLOG:%1 [FPS: %2|SERVERTHREADS: %3] (v0016F)',_this,diag_fps,count diag_activeSQFScripts] call FN_CALL_LOG_DLL;
	diag_log ('<infiSTAR.de>HL| '+_this+' (v0016F)');
	if(!isNil'MAR_fnc_log')then{[_this,'infiSTAR.de_HACKLOG'] call MAR_fnc_log;};
";
FNC_A3_SURVEILLANCELOG = compileFinal "
	format['A3_SURVEILLANCELOG:%1 [FPS: %2|SERVERTHREADS: %3] (v0016F)',_this,diag_fps,count diag_activeSQFScripts] call FN_CALL_LOG_DLL;
	diag_log ('<infiSTAR.de>SL| '+_this+' (v0016F)');
	if(!isNil'MAR_fnc_log')then{[_this,'infiSTAR.de_SURVEILLANCELOG'] call MAR_fnc_log;};
";
FNC_A3_ADMINLOG = compileFinal "
	format['A3_ADMINLOG:%1 (v0016F)',_this] call FN_CALL_LOG_DLL;
	diag_log ('<infiSTAR.de>AdminLog| '+_this+' (v0016F)');
	if(!isNil'MAR_fnc_log')then{[_this,'infiSTAR.de__ADMINLOG'] call MAR_fnc_log;};
";
FNC_A3_CONNECTLOG = compileFinal "
	format['A3_CONNECTLOG:%1 (v0016F)',_this] call FN_CALL_LOG_DLL;
	diag_log ('<infiSTAR.de>ConnectLog| '+_this+' (v0016F)');
";
FNC_A3_TOKENLOG = compileFinal "
	format['A3_TOKENLOG:%1 (v0016F)',_this] call FN_CALL_LOG_DLL;
	diag_log ('<infiSTAR.de>TokenLog| '+_this+' (v0016F)');
";
FNC_A3_RESOLVESTEAMNAMELOG = compileFinal "
	format['A3_RESOLVESTEAMNAMELOG:%1 (v0016F)',_this] call FN_CALL_LOG_DLL;
	diag_log ('<infiSTAR.de>ResolveSteamName| '+_this+' (v0016F)');
...
FNC_INFISERVERKICK = compileFinal ""
	...
	diag_log format['<infiSTAR.de>KickLog| %1(%2)   (v0016F)',_name,_uid];
"";
...
FNC_A3_infiSTARBAN = {
	...
	if(_reason == '')then
	{
		format['A3_BANLOG:%1(%2) (v0016F)',_name,_uid] call FN_CALL_LOG_DLL;
		diag_log format['<infiSTAR.de>BanLog| %1(%2)   (v0016F)',_name,_uid];
	}
	else
	{
		format['A3_BANLOG:%1(%2) %3 (v0016F)',_name,_uid,_reason] call FN_CALL_LOG_DLL;
		diag_log format['<infiSTAR.de>BanLog| %1(%2) %3 (v0016F)',_name,_uid,_reason];
	};
	if(isNil'"+_TMPBAN+"')then{"+_TMPBAN+"=[];}else{if(typeName "+_TMPBAN+"!='ARRAY')then{"+_TMPBAN+"=[];};};
	if(typeName "+_TMPBAN+"=='ARRAY')then
	{
...
_fnc_infiSTAR_PlayerLog = {
	...
	if(_uid != '')then
	{
		...
		[
			...
			'76561198052466790','76561198166992322',
			'0'
		];
...
_AdminReqReal = {
	...
	if(isNil '_clientNetID')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + 'AdminReq clientNetID is Nil (v0016F)';
		_log call FNC_A3_SURVEILLANCELOG;
	};
	if(typeName _clientNetID != 'STRING')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + format['AdminReq clientNetID wrong typeName: %1   %2 (v0016F)',typeName _clientNetID,_clientNetID];
		_log call FNC_A3_SURVEILLANCELOG;
	};
	if(_clientNetID == '')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + 'AdminReq clientNetID is empty (v0016F)';
		_log call FNC_A3_SURVEILLANCELOG;
	};
	_ObjFromNetID = objectFromNetId _clientNetID;
	if(isNil '_ObjFromNetID')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + 'AdminReq playerObj is Nil (v0016F)';
		_log call FNC_A3_SURVEILLANCELOG;
	};
	if(typeName _ObjFromNetID != 'OBJECT')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + format['AdminReq ObjFromNetID wrong typeName: %1   %2 (v0016F)',typeName _ObjFromNetID,_ObjFromNetID];
		_log call FNC_A3_SURVEILLANCELOG;
	};
	if(isNull _ObjFromNetID)exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + 'AdminReq ObjFromNetID is Null (v0016F)';
		_log call FNC_A3_SURVEILLANCELOG;
	};
	...
	if(isNil '_array')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + 'AdminReq is Nil (v0016F)';
		_log call FNC_A3_SURVEILLANCELOG;
	};
	if(typeName _array != 'ARRAY')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + format['AdminReq wrong typeName: %1   %2 (v0016F)',typeName _array,_array];
		_log call FNC_A3_SURVEILLANCELOG;
	};
	...
	if(isNil '_option')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + 'AdminReq option is Nil (v0016F)';
		_log call FNC_A3_SURVEILLANCELOG;
	};
	if(typeName _option != 'SCALAR')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + format['AdminReq option wrong typeName: %1   %2 (v0016F)',typeName _option,_option];
		_log call FNC_A3_SURVEILLANCELOG;
	};
	...
	if(typeName _playerObj != 'OBJECT')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + format['AdminReq playerObj wrong typeName: %1   %2 (v0016F)',typeName _playerObj,_playerObj];
		_log call FNC_A3_SURVEILLANCELOG;
	};
	if(isNull _playerObj)exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + 'AdminReq playerObj is Null (v0016F)';
		_log call FNC_A3_SURVEILLANCELOG;
	};
	if!(_ObjFromNetID isEqualTo _playerObj)then
	{
		_mytime = call fnc_getserverTime;
		_log = _mytime + format['AdminReq playerObj != _ObjFromNetID - %1/2  (v0016F)',_playerObj,_ObjFromNetID];
		_log call FNC_A3_SURVEILLANCELOG;
		_playerObj = _ObjFromNetID;
	};
	if(!isPlayer _playerObj)exitWith{
		_mytime = call fnc_getserverTime;
		_log = format['AdminReq playerObj is not a player   %1(%2) (v0016F)',name _playerObj,getPlayerUID _playerObj];
		_log = _mytime + _log;
		_log call FNC_A3_SURVEILLANCELOG;
	};
	...
	if(isNil '_tokenreceived')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + 'AdminReq tokenreceived is Nil (v0016F)';
		_log call FNC_A3_SURVEILLANCELOG;
	};
	if(typeName _tokenreceived != 'STRING')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + format['AdminReq tokenreceived wrong typeName: %1   %2 (v0016F)',typeName _tokenreceived,_tokenreceived];
		_log call FNC_A3_SURVEILLANCELOG;
	};
	if(_tokenreceived == '')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + 'AdminReq tokenreceived is empty (v0016F)';
		_log call FNC_A3_SURVEILLANCELOG;
	};
	...
	if(_clientUID isEqualTo '')then
	{
		...
		if(_foundtokenid > 0)then
		{
			...
			if(!isNil '_puidfound')then
			{
				...
				_mytime = call fnc_getserverTime;
				_log = _mytime + format['%1(%2) | AdminReq - SERVER RESOLVED UID! (v0016F)',_clientName,_clientUID];
				_log call FNC_A3_SURVEILLANCELOG;
			};
		};
	};
	...
	if(!_result)exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + format['%1(%2) | AdminReq - BadToken %3 (v0016F)',_clientName,_clientUID,_tokenreceived];
		_log call FNC_A3_SURVEILLANCELOG;
	};
	...
	_fnc_NoUID = {
		...
		_mytime = call fnc_getserverTime;
		_log = _mytime + format['%1(%2) | AdminReq - !!!!!!ADMIN-LOGIN-ERROR!!!!!! (v0016F)',_clientName,_clientUID];
		_log call FNC_A3_SURVEILLANCELOG;
	};
	call {
	...
	{
		_mytime = call fnc_getserverTime;
		_log = _mytime + format['%1(%2) | AdminReq - Attempted to Use the AdminMenu (v0016F)',_clientName,_clientUID];
		_log call FNC_A3_SURVEILLANCELOG;
	};
...
_FNC_AH_KICKLOGSPAWN =
{
	...
	if(isNil '_input')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + 'FNC_AH_KICKLOGSPAWN _this is Nil (v0016F)';
		_log call FNC_A3_SURVEILLANCELOG;
	};
	...
	if(isNil '_tokenreceived')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + '_tokenreceived _this is Nil (v0016F)';
		_log call FNC_A3_SURVEILLANCELOG;
	};
	if(typeName _tokenreceived != 'STRING')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + format['_tokenreceived wrong type %1 (v0016F)',typeName _tokenreceived];
		_log call FNC_A3_SURVEILLANCELOG;
	};
	...
	if(isNil '_arraysent')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + '_arraysent _this is Nil (v0016F)';
		_log call FNC_A3_SURVEILLANCELOG;
	};
	if(typeName _arraysent != 'ARRAY')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + format['_arraysent wrong type %1 (v0016F)',typeName _arraysent];
		_log call FNC_A3_SURVEILLANCELOG;
	};
	...
	if(typeName _netId != 'STRING')exitWith{
		_mytime = call fnc_getserverTime;
		_log = _mytime + format['_netId wrong type %1 (v0016F)',typeName _netId];
		_log call FNC_A3_SURVEILLANCELOG;
	};
			...
			if(_txt != '')then
			{
				...
				};
				{
					if(_txt find _x > -1)then{
						waitUntil {(findDisplay 24) closeDisplay 0;isNull findDisplay 24};
						_log = format['BadText: %1 [%2]',_txt,_x];
						[_name,_puid,'HLOG_SKICK',toArray(_log)] call _AHKickLog;
						(findDisplay 46)closeDisplay 0;
					};
...
for '_i' from 0 to (count _cfgPatches - 1) do
{
	...
	{
		_log = format['Bad Addon found in CfgPatches: %1 -> %2',configName _patchClass,_badClasses];
		[_name,_puid,'HLOG_SKICK',toArray(_log)] call _AHKickLog;
		[] spawn _AHKickOFF;
	};
};
...
[_name,_puid,_AHKickOFF,_AHKickLog] spawn {
private['_name','_puid','_AHKickOFF','_AHKickLog','_lastpos','_log','_lasttime','_difftime','_curpos','_tmpAHpos','_driver','_curNetID','_type','_allowTP','_fn_inTradeZone','_inTrader','_curVehicle'];
_name = _this select 0;
_puid = _this select 1;
_AHKickOFF = _this select 2;
_AHKickLog = _this select 3;
...
while{true}do
{
	if((!isNull player)&&{alive player}&&{((typeOf player) isEqualTo 'Exile_Unit_Player')})then
	{
		...
		if(_distance > 3)then
		{
			...
			if(_speed < (_topSpeed*1.8))exitWith{};
...
_devLog = format['%1 VERSION: 06112015 infiSTAR.de AHAT (v0016F) - server running: %2:223',VERSIONCHECKRESULT,call _GET_TIME_TIME];diag_log _devLog;
"; if(!_HIDE_FROM_PLAYERS)then{ _A3AHstring = _A3AHstring + "
